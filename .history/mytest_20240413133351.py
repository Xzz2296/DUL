...
class erf_loss(nn.Module):
    ...
    def forward(self, mu, std, label):
        ...
        for i in range(batch_size):
            batch_dist = torch.abs(weight_expanded - mu_expanded[i]) # C * dim
            y = batch_dist / torch.sqrt(torch.tensor(2.0))/(std_expanded[i] + 1e-8) # C * dim ; Y ~ N(0,1)
            one_hot_label = F.one_hot(label[i],num_classes=weight.shape[0]).unsqueeze(-1).float() # C *1
            positive = one_hot_label * y # C * dim
            negative = (1-one_hot_label) * y # C * dim
            erf_value = torch.erf(positive)  # C * dim
            erfc_value = torch.erfc(negative) # C * dim
            batch_loss = torch.sum(erf_value,dim=-1) + torch.sum(erfc_value,dim=-1)
            loss += batch_loss
            # 释放不再需要的张量
            del batch_dist, y, one_hot_label, positive, negative, erf_value, erfc_value, batch_loss
