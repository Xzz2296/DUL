...
class DUL_FR_Tester():
    ...
    def face_recog(self):
        ...
        for value in Test_FR_Data_Dict.values():
            ...
            accuracy, best_threshold, tpr, fpr = perform_face_recog(self.dul_args.multi_gpu, self.dul_args.embedding_size,
                                                                    self.dul_args.batch_size, BACKBONE, testdata, testdata_issame)
            print(value.upper(), ': ', accuracy, flush=True)
            torch.save(tpr, os.path.join('roc_curve', value.upper() + '_tpr.pth'))
            torch.save(fpr, os.path.join('roc_curve', value.upper() + '_fpr.pth'))
        print('=' * 60, flush=True)
