...
class My_Density(nn.Module):
    def __init__(self,Softmax_head):
        ...
        std: batch * embedding
        '''
        super(My_Density, self).__init__()
    ...
    def forward(self, mu, std):
        ...
        density = -torch.sum((weight_expanded - mu_expanded) ** 2 / (2.0 * std_expanded ** 2 + 1e-8), dim=2) - 0.5 * torch.log(torch.sum(std_expanded ** 2, dim=2) * 2.0 * torch.pi)

        # output = -(weight_expanded - mu_expanded) ** 2 / (2.0 * std_expanded ** 2 + 1e-8) - torch.log(std_expanded) -  0.5 * torch.log(torch.tensor(2 * torch.pi))
        # output_softmax = torch.nn.functional.softmax(output, dim=1)
        # return output_softmax
        return density
