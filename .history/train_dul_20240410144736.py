...
from head.metrics import ArcFace, CosFace, SphereFace, Am_softmax, Softmax, Density
from loss.focal import FocalLoss,Density_loss
...
class DUL_Trainer():
    ...
    def _dul_runner(self):
        ...
        DISP_FREQ = 8*len(train_loader) // 100 # frequency to display training loss & acc
        ...
        for epoch in range(self.dul_args.num_epoch):
            ...
            for inputs, labels in train_loader:
                ...
                loss = 0
                # x,mu_dul,std_dul =BACKBONE(inputs)
                mu_dul, std_dul = BACKBONE(inputs) # namely, mean and std
                print(f"mu_dul.size():{mu_dul.size()}")
                print(f"std_dul.size():{std_dul.size()}")
            ...
            # ----- save model
            if epoch==4 or epoch==7 or epoch==12 or epoch>20:
                print("=" * 60, flush=True)
