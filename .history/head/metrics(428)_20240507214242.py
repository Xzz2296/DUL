    ...
    def forward(self, mu, std):
        ...
        for i in range(mu.shape[0]):
            for j in range(weight.shape[0]):
                output[i, j] = -0.5*(torch.sum(((mu[i] - weight[j]) ** 2) / (std[i] ** 2)) +  torch.log(torch.sum(std[i])**2 * 2.0 * torch.pi))
        return output
        # weight = self.weight
        # density = torch.zeros(mu.shape[0], weight.shape[0])
        # for i in range(mu.shape[0]):
        #     mu_expanded = mu[i].unsqueeze(0)
        #     std_expanded_2 = std[i].unsqueeze(0) ** 2
        #     density[i] = -0.5 *(torch.sum((weight - mu_expanded) ** 2 / std_expanded_2, dim=1) + torch.log(torch.sum(std_expanded_2)) + self.bias)
        # return density
...
class Density_Softmax(nn.Module):
    ...
    def forward(self, mu, std):
        # 使用一个for循环计算概率密度, 原理和上面的MY_HEAD一样
        weight = self.weight
        density = torch.zeros(mu.shape[0], weight.shape[0])
        for i in range(mu.shape[0]):
            mu_expanded = mu[i].unsqueeze(0)
            ...
            density[i] = -0.5 *(torch.sum((weight - mu_expanded) ** 2 / std_expanded_2, dim=1) + torch.log(torch.sum(std_expanded_2)) + self.bias)
        # w = self.weight 
        # w_expanded= w.unsqueeze(0) # 1* C * dim
