...
class erf_loss(nn.Module):
    ...
    def forward(self, x, mu, std, label):
        ...
        distance_metric = torch.cdist(weight_expanded,mu_expanded).squeeze(-1) # batch *C *1
        one_hot_label = F.one_hot(label,num_classes=weight.shape[0]).unsqueeze(-1).float() # batch * C
