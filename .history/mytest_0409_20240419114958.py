...
class My_Density(nn.Module):
    ...
    def forward(self, mu, std):
        w = self.weight
        mu_expanded = mu.unsqueeze(0) # 1 * class * dim
        mu_expanded = mu.unsqueeze(1) # batch * 1 * dim
        std_expanded = std.unsqueeze(1) # batch * 1 * dim
        output = -torch.sum((mu_expanded - mu_expanded) ** 2, dim=2) / (2.0 * torch.sum(std_expanded ** 2, dim=2) + 1e-8) - torch.log(torch.sum(std_expanded, dim=2)) -  0.5 * torch.log(torch.tensor(2 * torch.pi))
        output_softmax = torch.nn.functional.softmax(output, dim=1)
        return output_softmax
