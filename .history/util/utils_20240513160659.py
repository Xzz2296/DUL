...
# Support: ['get_time', 'l2_norm', 'make_weights_for_balanced_classes', 'get_val_pair', 'get_val_data', \
#           'separate_irse_bn_paras', 'separate_resnet_bn_paras', 'warm_up_lr', 'schedule_lr', 'de_preprocess', \
#           'hflip_batch', 'ccrop_batch', 'gen_plot', 'perform_val', 'buffer_val', 'AverageMeter', 'accuracy', \
#           'add_gaussian_noise', 'get_data_pair', 'perform_face_recog']
...
class add_gaussian_noise(object):
    def __init__(self, mean=0.0, var=30, p=0.0):
        self.mean = mean
        self.var = var
        self.p = p
...
def perform_face_recog(multi_gpu, embedding_size, batch_size, backbone, carray, issame, nrof_folds = 10, tta = True, outfolder=''):
    if outfolder:
    ...
    with torch.no_grad():
        while idx + batch_size <= len(carray):
            ...
            if tta:
                ...
                # backbone return image_features and logvar, use image_features only here; besides, convert to cpu-tensor, then to numpy
