...
class My_HEAD(nn.Module):
    def __init__(self, weight):
        ...
        self.weight = weight
        self.bias =  torch.log(torch.tensor(100 / torch.pi))
    ...
    def forward(self, mu, std):
        ...
        for i in range(mu.shape[0]):
            for j in range(weight.shape[0]):
                #
                output[i, j] = -0.5*(torch.mean(((mu[i] - weight[j]) ** 2) / std[i]**2 + torch.log(std[i]**2))) + self.bias

        # 优化后的运算
        # 计算mu^2/std^2
        temp_mu = ((mu/std)**2).mean(dim=1).unsqueeze(1)
        # 计算weight^2/std^2
        temp_weight = F.linear(torch.reciprocal(std)**2 , weight**2) / mu.shape[1]
        # 计算mu*weight/std^2
        temp_mu_weight = 2 * F.linear(mu/std**2, weight) / mu.shape[1]
        density = -0.5*(temp_mu + temp_weight - temp_mu_weight + torch.mean(torch.log(std**2), dim=1).unsqueeze(1)) + self.bias
...
class Density_Softmax(nn.Module):
    ...
    def forward(self, mu, var):
        # 优化后的运算
        # 计算mu^2/std^2
        temp_mu = (mu**2 / var).mean(dim=1).unsqueeze(1)
        # 计算weight^2/std^2
        temp_weight = F.linear(torch.reciprocal(var) , weight**2) / mu.shape[1]
        # 计算mu*weight/std^2
        temp_mu_weight = 2 * F.linear(mu/var, weight) / mu.shape[1]
        density = -0.5*(temp_mu + temp_weight - temp_mu_weight + torch.mean(torch.log(var), dim=1).unsqueeze(1)) + self.bias
        return density
...
if __name__ == "__main__":
    ...
    std_dul = torch.tensor([[4,2,1], [1, 1, 2], [2, 2, 3]])
    HEAD = My_HEAD(weight)
    # outputs是两重for循环的结果，density是优化后的结果
    outputs, density = HEAD(mu_dul, std_dul)
    print(outputs)
    print(density)